...
#     _LOG.info('Dataset {} with arguments {}'.format(dataset_name, dataset_args))
    ...
    # 여기가 liver냐 cardiac이냐 등에 따라 dataset class를 가져오는 곳
...
#     dataset = lambda transform: obj(transform=transform, args=args)
    return obj(mode, args)


def setup_model(args):
    """
    Prepare model according to arg
    """
    available_models = get_available_classes(models, 'models.', '_MODEL_NAME')
    
    path = None
    if args.pretrained_model_path is not None:
        if args.pretrained_model_path[-3:] == 'tar':
            path = args.pretrained_model_path
        else:
            path = args.pretrained_model_path + '/model_best_{:02d}.pth.tar'.format(args.trial)
    print('path :', path)
    model = available_models[args.arch](pretrained=args.pretrained, checkpoint_path=path, input_depth=args.input_depth, num_classes=args.num_classes)
    return model.model
